...
class VOCDataset():
    ...
    def __init__(self, root_dir, default_boxes,
        ...
        self.idx_to_name = [
            'starling', 'bluebird', 'robin', 'squirrel',
            'titmouse', 'finch', 'dove', 'sparrow']
    ...
    def _get_annotation(self, index, orig_shape):
        ...
        h, w = orig_shape
        filename = self.ids[index]
        anno_path = os.path.join(self.anno_dir, os.path.splitext(filename)[0] + '.xml')
...
def create_batch_generator(root_dir, default_boxes,
    ...
    voc = VOCDataset(root_dir, default_boxes,
